"""
Lee logs/trayectoria.csv (generado por deteccion_movimiento.py) y grafica la
...
def graficar(trayectorias, out_path):
    ...
    ax.set_title("Trayectorias detectadas - Práctica 3", color=TINTA_PRIMARIA, fontsize=13, pad=12)
    ...
    legend = ax.legend(frameon=False, labelcolor=TINTA_PRIMARIA, loc="best")
...
def main():
    ...
    if not os.path.exists(csv_path):
        raise FileNotFoundError(
            f"No se encontro {csv_path}. Ejecuta primero deteccion_movimiento.py y guarda algunas capturas."
        )
